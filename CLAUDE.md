...
- **`ZLangRegistry.scala`** — Maps file extensions and human-readable language names to RSTA `SyntaxConstants` style strings. `forPath(path)` looks up by extension; `forLang(lang)` looks up by name alias (case-insensitive). Also maps extensions to LSP language IDs via `langIdFor(path)`. `autoWrapExts: Set[String]` lists extensions that trigger automatic word-wrap on `Get` (currently `md`, `markdown`). Used by `ZWnd`'s `Hilite` command and `ZLspSupport`.
