...
- **`ZColors.scala`** — Canonical colour constants shared across `ZWnd`, `ZPanel`, `ZCol`, `ZFuzzyPicker`, and `ZTheme`. Named by role, not by hue (`TagBack`, `TagFore`, `TagCaret`, `TagSelBack`, `BodyBack`, `BodySelBack`). Changing a constant here propagates everywhere.
