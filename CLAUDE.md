...
- **`ZUtilities.scala`** — Static utilities: text selection logic, brace/symbol matching (`symMatch`), external process launching (`extCmd`), independent z-instance spawning (`spawnZ(dir)`), shell command tokenization (handles single-quoted tokens), and shared split-pane divider helpers (`collectDividers`/`applyDividers`) used by both `ZPanel` and `ZCol`. `spawnZ` discovers the running jar via `getProtectionDomain.getCodeSource`, finds the java executable via `ProcessHandle`, launches a detached child process with stdout/stderr discarded; no-ops when not running from a fat jar.
...
- **`ZLangRegistry.scala`** — Maps file extensions and human-readable language names to RSTA `SyntaxConstants` style strings. `forPath(path)` looks up by extension; `forLang(lang)` looks up by name alias (case-insensitive). Also maps extensions to LSP language IDs via `langIdFor(path)`. `autoWrap(path: String): Boolean` returns true for extensions that trigger automatic word-wrap on `Get` (currently `md`, `markdown`). Used by `ZWnd` on `Get` and `ZLspSupport`.
...
- **`ZMarkdownTokenMaker.scala`** — Extends RSTA's `MarkdownTokenMaker`. `getTokenList` calls `super` then walks the token chain in a single post-processing pass, retyping tokens so every Markdown element gets an unambiguous slot: H1→`RESERVED_WORD`, H2→`RESERVED_WORD_2`, H3+→`DATA_TYPE`, bold→`FUNCTION`, italic→`VARIABLE`, bold+italic→`COMMENT_KEYWORD`, fenced code body→`LITERAL_BACKQUOTE`, fence delimiter→`LITERAL_STRING_DOUBLE_QUOTE`, lang specifier→`LITERAL_CHAR`. Registered as the `text/markdown` handler in `z.scala` startup before any window is opened.
- **`ZMarkdownTheme.scala`** — Singleton holding mutable font slots for Markdown elements (`h1Font`, `h2Font`, `h3Font`, `boldFont`, `emFont`, `boldItalFont`, `codeFont`, `quoteFont`). `null` slots derive from the body font at render time. Defaults: Vera Serif Bold 18/15/14pt for headings, Hack for code. `setFont(elem, family, size)` is the write path (called by `MdFont` command). Persisted as `md.font.*` / `md.font.*.size` keys in `~/.z/settings`.
- **`ZFuzzyPicker.scala`** — Keyboard-driven fuzzy file picker invoked by Ctrl+P. `show(root, parent, initialQuery, relativeTo)` is the entry point. Walks the directory tree in a background thread (`CopyOnWriteArrayList` + `AtomicInteger`), skipping common build/cache dirs. Scores matches by consecutive runs, filename position, segment-start hits, and path length. Supports path-prefix queries (`/`, `~/`, `./`, `../`) that re-root the walk after a 150 ms debounce; the prefix is stripped from the query field after the root changes. `relativeTo` (passed as `baseDir` from `ZWnd`, derived from `ZWnd.root`) controls the relativization base for returned paths — ensures the path resolves correctly when passed to `look()` (tag invocations) or inserted as text (body invocations). When invoked from a `ZWnd`, the search `root` is `colRoot` (col's `currentDir` captured at window construction), not `ZWnd.root` — so the walk covers the full column scope regardless of how deep the open file is.
...
`ZTextArea` has a **capture mode** API: `startCapture()`, `endCapture(): String`, `abortCapture()`. All three tag-line levels support capture mode:
- `ZWnd` manages state via `captureTA: Option[ZTextArea] = None` (presence means active); exposes `cancelCapture()`.
- `ZPanel` and `ZCol` each manage state via `var captureActive: Boolean`.

- **Ctrl+Enter** — if text is selected: executes it as a command (no deletion). If capture is active: ends capture and executes. Otherwise: starts capture, recording the caret position. Everything typed from that point is highlighted via a Swing `Highlighter` (using `peer.getSelectionColor` so it follows the active theme). The `Highlighter` + `CaretListener` approach is synchronous (no `invokeLater`), no flicker. Cascade semantics: app tag runs captured command in every column via `cols.foreach(_.command(txt))`; col tag runs in every window via `wnds.foreach(_.command(txt))`; window tag runs in that window only.
- **Ctrl+F** — if text is selected: performs a look on it (no deletion). If capture is active: ends capture and performs a look. No-op outside capture with no selection. Cascade semantics: app tag looks in every column via `cols.foreach(_.look(txt))`; col tag looks across all windows via `look(txt)` (traverse=true); window tag looks in that window only.
...
- **Ctrl+P** opens the file-chooser path picker in `ZWnd`, `ZCol`, and `ZPanel`. When invoked from a tag line the selected file is opened immediately via `look()` (no path inserted into the tag); when invoked from the body the path is inserted as text at the cursor.
...
The `indHilite` flag is persisted in `Dump`/`Load` and re-applied automatically on `Get` when the file path changes. On `Get`, if the file has a known extension and `indHilite` is not already set, highlighting is **auto-enabled** (and the z theme applied) without any explicit `Hilite` command — so `.md`, `.scala`, `.go`, etc. all light up on first open. Markdown files also auto-enable word-wrap. Adding new languages means adding entries to `ZLangRegistry.byExt` and/or `byName`.

**Markdown-specific typography** is handled by `ZMarkdownTokenMaker` + `ZMarkdownTheme` + `ZTheme.applyMarkdownOverrides`:
- `ZTheme.apply` calls `applyMarkdownOverrides(ta)` after every base theme when `getSyntaxEditingStyle() == "text/markdown"` — so all themes (z, dark, monokai, etc.) automatically get correct Markdown font treatment.
- `applyMarkdownOverrides` reads font slots from `ZMarkdownTheme` and overlays them on the scheme's existing colors; heading fonts come from the theme, typography from the slots.
- **`MdFont elem 'Family' size`** (app tag line) — updates a `ZMarkdownTheme` slot and cascades `Color z` to all windows. Elements: `h1 h2 h3 bold em bolditalic code quote`. Persisted as `md.font.*` keys in `~/.z/settings`.
...
- **Markdown fonts** — `md.font.<elem>` and `md.font.<elem>.size` are read at startup into `ZMarkdownTheme` slots and written on exit for any non-null slot. Elements: `h1 h2 h3 bold em bolditalic code quote`.
